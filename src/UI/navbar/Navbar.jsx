                ...
                <nav className={cl['navbar']}>
                    <ul className={cl['navbar__links']}>
                        <li className={cl['navbar__links-item']}>
                            <button className={`${cl['navbar__link']} ${cl['navbar__link-main']} ${cl['navbar__link_active']}`} onClick={() => route(`/main${getParams()}`)}>Новгородская  еда</button>
                        </li>
                        <li className={cl['navbar__links-item']}>
                            <button className={cl['navbar__link']} onClick={() => route(`/restaurants${getParams()}`)}>Все рестораны</button>
                        </li>
                        <li className={cl['navbar__links-item']}>
                            <button className={cl['navbar__link']} onClick={() => route(`/map${getParams()}`)}>Карта ресторанов</button>
                        </li>
                    </ul>

                    <ul className={cl["navbar__lang-switcher"]}>
                        <li className={cl["navbar__item"]}><Link className={cl["navbar__lang-link"]} to={`${pathname}?lang=ru`}>RU</Link></li>
                        <li className={cl["navbar__item"]}><span className={cl["navbar__lang-switcher-separator"]}></span></li>
                        <li className={cl["navbar__item"]}><Link className={cl["navbar__lang-link"]} to={`${pathname}?lang=en`}>EN</Link></li>
                    </ul>
                </nav>
            </header>
    )} else {
            ...
            <header className="header" role="banner">
                ...
                <ul className="navbar__menu" id="menu" tabIndex="-1" aria-label="main navigation" hidden>
                    <li className="navbar__item">
                        <button className="navbar__link navbar__link-main navbar__link_active" data-url={`/main${getParams()}`}>Новгородская  еда</button>
                    </li>
                    <li className="navbar__item">
                        <button className="navbar__link" data-url={`/restaurants${getParams()}`}>Все рестораны</button>
                    </li>
                    <li className="navbar__item">
                        <button className="navbar__link" data-url={`/map${getParams()}`}>Карта ресторанов</button>
                    </li>
                </ul>
